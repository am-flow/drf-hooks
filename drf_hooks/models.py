        ...
        for event_name, auto in settings.HOOK_EVENTS.items():
            ...
            if action in __EVENT_LOOKUP[model]:
                raise ImproperlyConfigured(
                    "settings.HOOK_EVENTS has a duplicate {action} for model " "{model}".format(
...
def get_hook_model():
    ...
    if __HOOK_MODEL is None:
        ...
        try:
            ...
            raise ImproperlyConfigured(
                "HOOK_CUSTOM_MODEL refers to unknown model '%s'" % model_label
            )
    return __HOOK_MODEL
...
class AbstractHook(models.Model):
    ...
    def deliver_hook(self, serialized_hook):
        ...
        get_client().post(url=self.target, data=serialized_hook, headers=self.headers)
